...
the_player = Player('Images/harley.png',100,100,screen)

# Make an alien/bad_guy
...
pygame.mixer.music.load('Sounds/arcade_music.wav')
pygame.mixer.music.play(-1) # Don't stop playing
pygame.mixer.music.set_volume(0.3) # Set the volume
...
while game_on: #will run forever (until break)
	...
	for event in pygame.event.get():
		...
		if event.type == pygame.QUIT:
			...
			if event.key == 275:
				...
				fire = pygame.mixer.Sound("Sounds/laser.aiff")
	...
	if(bullet_hit):
		# If one alien is killed you gain 1 point
		points += 1
	...
	if (power_up):
		recharge = pygame.mixer.Sound("Sounds/recharge.wav")
		recharge.play(0) # Play sound once
		recharge.set_volume(1) # Set sound volume
		player.health += 3 # Harley gains 3 health points!
		print "Harley has %d health points!" % (player.health)
